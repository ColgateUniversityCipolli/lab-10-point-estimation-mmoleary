...
``{r}
ggdat <- tibble( x = 0:11 ) |>
  group_by(theta = c(0.5, 1, 1.5)) |>
mutate(PMF = daltbinom(x = x, size = 11, prob = 0.5, theta = c(0.5, 1, 1.5)))

PMF.plot <- ggplot(ggdat) +
  geom_linerange(data = ggdat,
  aes(x = 0:11, ymin = 0, ymax = PMF)) +
  geom_hline(yintercept = 0) +
...
The graphical summary demonstates that the proportion of winning is relatively close between Littler and Humphries, Smith and Gerwen and Aspinall, and Price adn Cross. Wright's Win percentage is much lower than any other player.
