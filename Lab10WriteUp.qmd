...
```{r}
ggdat <- tibble(theta = c(0.5, 1, 1.5)) |>
mutate(PMF = daltbinom(x = 6, size = 11, prob = 0.5, theta = theta))

PMF.plot <- ggplot(ggdat) +
  geom_linerange(data = ggdat,
  aes(x = theta, ymin = 0, ymax = PMF)) +
  geom_hline(yintercept = 0) +
  scale_x_continuous("Theta Values", breaks = seq(0.5, 1, 1.5))+
  ylab("P(X = 6)") +
...
dart.data.cleaned = dart.data |> dplyr::filter(Score != "w/o")
```
